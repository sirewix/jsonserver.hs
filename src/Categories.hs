...
getCategories mbcid (log, db) = catchDb log (return BadRequest) $ do
...
createCategory :: UserName -> (Name, Maybe CategoryId) -> Endpoint
...
editCategory (UserName admin) (Name category, CategoryId cid) = execdb
  "UPDATE categories SET name = ? WHERE id = ?"
  (category, cid)
  (Just $ admin <> " changed category " <> showText cid <> " to '" <> category <> "'")

deleteCategory (UserName admin) (CategoryId cid) =
